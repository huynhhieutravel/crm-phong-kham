...
$page_title = __('menu.dashboard') . ' / Hoạt động trong ngày';
...
// --- 1. Top Metrics ---
$stmt_app = $db->query("SELECT COUNT(*) FROM appointments WHERE DATE(appointment_date) = '$today'");
$total_appointments = $stmt_app->fetchColumn();

$stmt_checkin = $db->query("SELECT COUNT(*) FROM appointments WHERE DATE(appointment_date) = '$today' AND status IN ('arrived', 'in_progress', 'completed')");
$total_checked_in = $stmt_checkin->fetchColumn();

$stmt_rev = $db->query("SELECT SUM(amount) FROM transactions WHERE type = 'income' AND DATE(transaction_date) = '$today'");
...
<div style="display: flex; justify-content: space-between; align-items: flex-end; margin-bottom: 2rem;">
    <div>
        <h2 style="font-size: 1.5rem; font-weight: 800; color: var(--text-main); margin-bottom: 0.25rem;">Hoạt động trong ngày</h2>
        <p style="color: var(--text-muted); font-size: 0.9rem;">Tổng hợp chi tiết lịch trình và hoạt động ngày hôm nay (<?php echo $display_date; ?>).</p>
    </div>
...
<div class="grid-stats" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: 1.5rem; margin-bottom: 2rem;">
    ...
    <div class="card stat-card">
        ...
        <div class="stat-label" style="color: var(--text-muted); font-size: 0.875rem; font-weight: 500;">Bệnh nhân đã đến</div>
    ...
    <div class="card stat-card">
        ...
        <div class="stat-label" style="color: var(--text-muted); font-size: 0.875rem; font-weight: 500;">Doanh thu dự kiến/Thực thu</div>
...
<div class="grid-content" style="display: grid; grid-template-columns: 2fr 1fr; gap: 1.5rem;">
    ...
    <div class="card">
        <h3 style="font-weight: 700; margin-bottom: 1.5rem; padding-bottom: 1rem; border-bottom: 1px solid var(--border-color);">
            <i class="fas fa-clipboard-list" style="color: var(--primary);"></i> Lịch trình chi tiết hôm nay
        </h3>
                ...
                <table class="table" style="width: 100%;">
                    <thead>
                        <tr style="text-align: left; border-bottom: 1px solid var(--border-color); color: var(--text-muted); font-size: 0.85rem;">
                            <th style="padding: 1rem 0.5rem; white-space: nowrap;">Giờ hẹn</th>
                            <th style="padding: 1rem 0.5rem; white-space: nowrap;">Bệnh nhân</th>
                            <th style="padding: 1rem 0.5rem; white-space: nowrap;">Loại khám</th>
                            <th style="padding: 1rem 0.5rem; white-space: nowrap;">Bác sĩ / KTV</th>
                            <th style="padding: 1rem 0.5rem; white-space: nowrap;">Trạng thái</th>
                            <th style="padding: 1rem 0.5rem; text-align: right; white-space: nowrap;">Thao tác</th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr style="border-bottom: 1px solid #f1f5f9; <?php echo $is_past ? 'opacity: 0.7;' : ''; ?>">
                            ...
                            <td style="padding: 1rem 0.5rem; vertical-align: middle;">
                                ...
                                <div style="display: flex; align-items: center; gap: 0.5rem; flex-wrap: wrap;">
                                    ...
                                    <?php if($a['history_count'] == 0): ?>
                                        <span style="font-size: 0.65rem; padding: 0.15rem 0.4rem; background: #fee2e2; color: #dc2626; border-radius: 4px; font-weight: 600; white-space: nowrap;">Mới</span>
                                    <?php endif; ?>
                                </div>
                            </td>
                            <td style="padding: 1rem 0.5rem; vertical-align: middle; white-space: nowrap;">
                                <div style="font-size: 0.85rem; color: var(--text-main); font-weight: 500;">
                                    <?php echo ($a['type'] == 're_exam') ? 'Tái khám' : (($a['type'] == 'treatment') ? 'Điều trị' : 'Khám mới'); ?>
                                </div>
                            </td>
                            ...
                            <td style="padding: 1rem 0.5rem; vertical-align: middle;">
                                <div style="font-size: 0.75rem; padding: 0.35rem 0.75rem; border-radius: 20px; font-weight: 600; background: <?php echo $color['bg']; ?>; color: <?php echo $color['color']; ?>; display: inline-flex; align-items: center; gap: 0.35rem; white-space: nowrap;">
                                    <?php 
                                        if($a['status'] == 'scheduled') echo '<i class="far fa-calendar"></i><span>Đã hẹn</span>';
                                        elseif($a['status'] == 'arrived') echo '<i class="fas fa-walking"></i><span>Đã đến</span>';
                                        elseif($a['status'] == 'in_progress') echo '<i class="fas fa-spinner fa-spin"></i><span>Đang khám</span>';
                                        elseif($a['status'] == 'completed') echo '<i class="fas fa-check"></i><span>Hoàn thành</span>';
                                        elseif($a['status'] == 'cancelled') echo '<i class="fas fa-times"></i><span>Đã hủy</span>';
                                    ?>
                                </div>
                                <?php if($is_past): ?>
                                    <div style="font-size: 0.7rem; color: #ef4444; font-weight: 600; margin-top: 6px; white-space: nowrap;">Trễ hẹn</div>
                                <?php endif; ?>
                            </td>
                            <td style="padding: 1rem 0.5rem; vertical-align: middle;">
                                <div style="display: flex; gap: 0.5rem; justify-content: flex-end; align-items: center;">
                                    <?php if($a['status'] == 'scheduled'): ?>
                                        <form action="/modules/appointments/checkin.php" method="POST" style="margin: 0;">
                                            ...
                                            <button type="submit" class="btn btn-sm" style="background: #10b981; color: white; border-radius: 6px; padding: 0.35rem 0.6rem; border: none; cursor: pointer; display: flex; align-items: center;" title="Check-in Bệnh nhân">
                                                <i class="fas fa-sign-in-alt"></i>
                                            </button>
                                        </form>
                                    <?php endif; ?>
                                    <a href="/modules/appointments/edit.php?id=<?php echo $a['id']; ?>" class="btn btn-sm btn-icon" style="background: #f8fafc; color: var(--text-main); border: 1px solid #e2e8f0; border-radius: 6px; padding: 0.35rem 0.6rem; display: flex; align-items: center; justify-content: center;" title="Chỉnh sửa">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <a href="/modules/patients/view.php?id=<?php echo $a['patient_id']; ?>" class="btn btn-sm btn-icon" style="background: #f8fafc; color: var(--text-main); border: 1px solid #e2e8f0; border-radius: 6px; padding: 0.35rem 0.6rem; display: flex; align-items: center; justify-content: center;" title="Hồ sơ bệnh nhân">
    ...
    <div style="display: flex; flex-direction: column; gap: 1.5rem;">
        ...
        <div class="card">
            <h3 style="font-weight: 700; margin-bottom: 1rem; padding-bottom: 1rem; border-bottom: 1px solid var(--border-color);">
                <i class="fas fa-user-md" style="color: #8b5cf6;"></i> Phân bổ Bác sĩ / KTV
            </h3>
            
            <?php if(empty($staff_utilization)): ?>
                <p style="color: var(--text-muted); font-size: 0.9rem; text-align: center; padding: 1rem 0;">Chưa phân bổ lịch hẹn nào.</p>
            <?php else: ?>
                ...
                <?php
                $unassigned_stmt = $db->query("SELECT COUNT(*) FROM appointments WHERE DATE(appointment_date) = '$today' AND doctor_id IS NULL OR doctor_id = 0");
                ...
                <div style="margin-top: 1rem; padding: 0.75rem; background: #fff1f2; border: 1px solid #ffe4e6; border-radius: 8px; display: flex; justify-content: space-between; align-items: center;">
                    <div style="color: #be123c; font-weight: 600; font-size: 0.85rem;">
                        <i class="fas fa-exclamation-circle"></i> Chưa xếp Bác sĩ/KTV
                    </div>

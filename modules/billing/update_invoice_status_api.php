...
try {
    ...
    if ($new_status === 'cancelled') {
        ...
        if ($invoice['transfer_amount'] > 0) $db->prepare("INSERT INTO transactions (type, category, amount, reference_id, description, branch_id, created_by) VALUES ('income', 'Chuyển khoản (Cũ)', ?, ?, ?, ?, ?)")->execute([$invoice['transfer_amount'], $invoice_id, "Phiếu tính tiền $inv_no", $branch_id, $creator_id]);
    }
